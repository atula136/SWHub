...
class MainViewController: TabBarViewController, ReactorKit.View {
    ...
    override func viewDidLoad() {
        ...
        if #available(iOS 10.0, *) {
            themeService.rx
                .bind({ $0.textDarkColor }, to: self.tab.tabBar.rx.unselectedItemTintColor)
                .disposed(by: self.rx.disposeBag)
        }
        themeService.typeStream.delay(.milliseconds(10), scheduler: MainScheduler.instance).subscribe(onNext: { [weak self] theme in
            ...
            if let items = self.tab.tabBar.items {
                let color = theme.associatedObject.textDarkColor
...
//        }).disposed(by: self.disposeBag)
    ...
    func viewController(with key: MainKey) -> BaseViewController {
        ...
        case .home:
            ...
            viewController?.tabBarItem.image = FontAwesomeIcon.homeIcon.image(ofSize: s28, color: .textDark).original
        ...
        case .message:
            ...
            viewController?.tabBarItem.image = FontAwesomeIcon.commentsIcon.image(ofSize: s28, color: .textDark).original
        ...
        case .search:
            ...
            viewController?.tabBarItem.image = FontAwesomeIcon.searchIcon.image(ofSize: s28, color: .textDark).original
        ...
        case .activity:
            ...
            viewController?.tabBarItem.image = FontAwesomeIcon.bellIcon.image(ofSize: s28, color: .textDark).original
        ...
        case .setting:
            ...
            viewController?.tabBarItem.image = FontAwesomeIcon.cogIcon.image(ofSize: s28, color: .textDark).original
        ...
        if let item = viewController?.tabBarItem {
            if #available(iOS 10.0, *) {
                ...
                themeService.rx
                    .bind({ [NSAttributedString.Key.foregroundColor: $0.textDarkColor] }, to: item.rx.titleTextAttributes(for: .normal))
