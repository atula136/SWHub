...
class TabBarController: SWFrame.TabBarController, ReactorKit.View {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.viewControllers = self.reactor?.currentState.keys.map { NavigationController(rootViewController: self.viewController(with: $0)) }
        themeService.rx
            .bind({ $0.dimColor }, to: self.tabBar.rx.barTintColor/*[self.tabBar.rx.barTintColor, self.safeBottomView.rx.backgroundColor]*/)
            .bind({ $0.tintColor }, to: self.tabBar.rx.tintColor)
